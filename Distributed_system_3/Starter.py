...
parser.add_argument('--c', type=int, default=1, help='Initial value for c')
...
while True:
    print(f'Running {c} coordinator nodes...')
    for id in range(c):
        processes.append(subprocess.Popen([sys.executable, 'Coordinator Node.py', id == 0, f'{id}', (
            id - 1) % c, (id + 1) % c, str(variables['x']), str(variables['y']), str(variables['z'])]))
